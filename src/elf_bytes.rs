...
mod interface_tests {
    ...
    #[test]
    fn aarch64_core_file_with_0_aligned_note() {
        // ELF header: 64bit, lsb, core, aarch64, one program header, no section headers
        // Program header: PT_NOTE, alignment 0
        // Note: NT_ARM_SYSTEM_CALL, name=LINUX\0
        let core = [
            0x7f, 0x45, 0x4c, 0x46, 0x02, 0x01, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
            0x00, 0x00, 0x04, 0x00, 0xb7, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
            ...
            0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x06, 0x00, 0x00, 0x00, 0x04, 0x00,
            0x00, 0x00, 0x04, 0x04, 0x00, 0x00, 0x4c, 0x49, 0x4e, 0x55, 0x58, 0x00, 0x00, 0x00,
            0xff, 0xff, 0xff, 0xff,
        ];

        let file = ElfBytes::<AnyEndian>::minimal_parse(&core).expect("should parse");
        let headers = file.segments().expect("should have one segment");
        assert_eq!(1, headers.len());
        let header = headers.get(0).expect("header should parse");
        let notes: Vec<Note<'_>> = file
            .segment_data_as_notes(&header)
            .expect("should be able to get data")
            .collect();
